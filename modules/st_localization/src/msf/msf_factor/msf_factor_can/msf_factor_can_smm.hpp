...
class MSFFactorCANSMM : public MSFFactorCAN {
 public:
  MSFFactorCANSMM() {
    Hx_.setZero();
    Hdx_.setZero();
  ...
  Eigen::VectorXd EvaluateRes(const VectorXd& current_state) override {
    ...
    // evaluate the error
    Eigen::Vector3d dp = p_obs_ - current_state.segment<3>(kcStatePosition);
    Eigen::Vector4d q_vec = current_state.segment<4>(kcStateQuat);
    Eigen::Quaterniond q_predict = Utility::HamiltonVecToEigenQ(q_vec);
    Eigen::Quaterniond dq = q_predict.conjugate() * q_obs_;
 ...
 private:
  // semantic map_matching provides 3d position and 3d attitude
  // in total 6d observation
