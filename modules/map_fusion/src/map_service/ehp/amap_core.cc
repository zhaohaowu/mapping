...
bool AmapAdapter::Init() {
  ...
  std::string data_root_path =
      FLAGS_map_dir + "/../amap_all/hd_map";  // 数据根路径//NOLINT
#ifdef ISMDC
...
bool AmapAdapter::Process(
  ...
  //     adc_trajectory.trajectory_point_size() > 0) {
  //   hozon::common::TrajectoryPoint point;
  //   int zone = -1;
  //   if (adc_trajectory.is_vehicle_reference_frame() &&
  //       adc_trajectory.debug().has_localization()) {
  //     point.mutable_path_point()->set_x(
  //         adc_trajectory.debug().localization().pose().position().x());
  //     point.mutable_path_point()->set_y(
  //         adc_trajectory.debug().localization().pose().position().y());
  //     point.mutable_path_point()->set_theta(
  //         adc_trajectory.debug().localization().pose().heading());
  //     zone = adc_trajectory.debug().localization().pose().using_utm_zone();
  //   } else {
  //     zone = adc_trajectory.utm_zone_id();
  //     point = adc_trajectory.trajectory_point(0);
  //   }
  //   double lon = point.path_point().x();
  //   double lat = point.path_point().y();
  //   hozon::common::coordinate_convertor::UTM2GCS(zone, &lon, &lat);
  //   localization.mutable_pose()->mutable_gcj02()->set_x(lon);
  //   localization.mutable_pose()->mutable_gcj02()->set_y(lat);
  //   localization.mutable_pose()->set_heading(point.path_point().theta());
  //   localization.mutable_pose()->mutable_wgs()->set_x(lon);
  //   localization.mutable_pose()->mutable_wgs()->set_y(lat);
  //   localization.mutable_pose()->mutable_linear_velocity()->set_x(point.v());
