...
#include "modules/local_mapping/lib/laneline/tracker/bev_lane_tracker/data_fusion/point_manager.h"
...
void AdaptorPointManager::DelPointsFar(
  ...
  if (count_track_over_dect >= 3) {
    ...
    int del_pt_size = 0, del_pt_index = -1;
    ...
    int count_inter_points = del_pt_size;
    // 近处开始插值点
    X_SWAP_.setZero();
    P_SWAP_.setZero();
    int index = 0;
...
void AdaptorPointManager::Process(Eigen::Matrix<double, 40, 1>* XPtr,
  ...
  UpdatePointsFar(measurement_points);
  // 3. 跟踪远端多次超过检测点，进行删除
  DelPointsFar(measurement_points);
