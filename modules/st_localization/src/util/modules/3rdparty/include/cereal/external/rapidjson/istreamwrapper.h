...
#ifdef _MSC_VER
CEREAL_RAPIDJSON_DIAG_PUSH
CEREAL_RAPIDJSON_DIAG_OFF(4351)  // new behavior: elements of array 'array' will
                                 // be default initialized
CEREAL_RAPIDJSON_DIAG_OFF(
    4127)  // ignore assert(false) for triggering exception
#endif
...
class BasicIStreamWrapper {
 public:
    ...
    Ch Peek() const {
        ...
        return CEREAL_RAPIDJSON_LIKELY(c != StreamType::traits_type::eof())
                   ? static_cast<Ch>(c)
                   : '\0';
    }
    ...
    const Ch* Peek4() const {
        CEREAL_RAPIDJSON_ASSERT(sizeof(Ch) ==
                                1);  // Only usable for byte stream.
        int i;
        bool hasError = false;
        for (i = 0; i < 4; ++i) {
            typename StreamType::int_type c = stream_.get();
