...
class ElementTrafficSignDB
    : public ElementDBBase<PerceptTrafficSign, TrafficSign> {
 public:
  DEFINE_SMART_PTR(ElementTrafficSignDB)

  ElementTrafficSignDB() = default;
  ElementTrafficSignDB(const CameraModel::Ptr& camera_model,
                       const SE3d& T_veh_cam);
  ~ElementTrafficSignDB() override {}
  ...
  size_t GetTrackerNumber() const { return sign_trackers_.size(); }

 private:
  // @brief: get stable fused signs from trackers
  adLocStatus_t GetStableSigns();

  // @brief: update tracked data by new perception
  adLocStatus_t UpdateTrackedPerceptData(const id_t tracker_id,
                                         const PerceptTrafficSign& new_percept);
 ...
 private:
  ...
  // sign trackers
  DepthTracker::PtrUMap sign_trackers_;
  ...
  std::unordered_map<id_t, std::shared_ptr<PerceptTrafficSign>>
      tracked_sign_data_;
};
