...
#include "modules/map_fusion_02/modules/lane/virtual_lane_construct/virtual_line_generate.h"
...
bool VirtualLineGen::ConstructVirtualLine(std::vector<Group::Ptr>* groups) {
  ...
  for (int i = 0; i < groups->size(); ++i) {
    ...
    if (i > 0) {
      if (Dist(groups->at(i - 1)->end_slice.po, curr_grp->start_slice.po) <
          1.0) {
    ...
    if (i < groups->size() - 1) {
      if (Dist(groups->at(i + 1)->start_slice.po, curr_grp->end_slice.po) <
          1.0) {
    ...
    for (int line_right = 1; line_right < curr_grp_lines.lines.size();
      ...
      // HLOG_INFO << "curr group id is " << curr_grp->str_id
