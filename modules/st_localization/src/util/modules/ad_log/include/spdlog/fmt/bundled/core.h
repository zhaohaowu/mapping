...
#define FMT_TYPE_CONSTANT(Type, constant) \
    ...
    struct type_constant<Type, Char>      \
        : std::integral_constant<type, constant> {}
...
struct arg_mapper {
    ...
    template <typename T,
              FMT_ENABLE_IF(std::is_constructible<basic_string_view<char_type>,
                                                  T>::value &&
                            !is_string<T>::value)>
    FMT_CONSTEXPR basic_string_view<char_type> map(const T& val) {
 ...
 public:
    class handle {
     public:
        ...
        void format(basic_parse_context<char_type>& parse_ctx,
                    Context& ctx) const {
...
class basic_format_args {
 ...
 private:
    ...
    internal::type type(int index) const {
        int shift = index * 4;
        return static_cast<internal::type>((types_ & (0xfull << shift)) >>
                                           shift);
    }
...
make_args_checked(const S& format_str,
    ...
    check_format_string<remove_const_t<remove_reference_t<Args>>...>(
        format_str);
    return {args...};
}
...
OutputIt vformat_to(OutputIt out,
                    ...
                    basic_format_args<buffer_context<Char>> args) {
    using container =
        remove_reference_t<decltype(internal::get_container(out))>;
