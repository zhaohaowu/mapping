...
class GenericRegex {
 ...
 private:
        ...
        while (ds.Peek() != 0) {
            switch (codepoint = ds.Take()) {
                ...
                case '[': {
                    ...
                    SizeType s =
                        NewState(kRegexInvalidState, kRegexInvalidState,
    ...
    bool Eval(Stack<Allocator>& operandStack, Operator op) {
        switch (op) {
            case kConcatenation:
                CEREAL_RAPIDJSON_ASSERT(operandStack.GetSize() >=
                                        sizeof(Frag) * 2);
                {
    ...
    bool AddState(Stack<Allocator>& l, SizeType index) const {
        ...
        return s.out == kRegexInvalidState;  // by using PushUnsafe() above, we
                                             // can ensure s is not validated
                                             // due to reallocation.
    }
