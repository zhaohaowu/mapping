...
/*! \def CEREAL_RAPIDJSON_NAMESPACE
   ...
   { #define CEREAL_RAPIDJSON_NAMESPACE_END   } } #include "rapidjson/..."
    \endcode
...
/*! \def CEREAL_RAPIDJSON_NO_INT64DEFINE
   ...
   types to be available at global scope.
    ...
    If users have their own definition, define CEREAL_RAPIDJSON_NO_INT64DEFINE
   to prevent RapidJSON from defining its own types.
*/
...
/*!
   ...
   other compilers may not have this. User can define CEREAL_RAPIDJSON_ENDIAN to
   either \ref CEREAL_RAPIDJSON_LITTLEENDIAN or \ref CEREAL_RAPIDJSON_BIGENDIAN.
...
/*! \ingroup CEREAL_RAPIDJSON_CONFIG
   ...
   platforms. User can customize by defining the CEREAL_RAPIDJSON_ALIGN function
   macro.
*/
...
#define CEREAL_RAPIDJSON_GETPOINTER(type, p) \
    (reinterpret_cast<type *>(               \
        reinterpret_cast<uintptr_t>(p) &     \
        static_cast<uintptr_t>(              \
            CEREAL_RAPIDJSON_UINT64_C2(0x0000FFFF, 0xFFFFFFFF))))
#else
...
#define CEREAL_RAPIDJSON_DIAG_PRAGMA(x) \
    CEREAL_RAPIDJSON_PRAGMA(GCC diagnostic x)
