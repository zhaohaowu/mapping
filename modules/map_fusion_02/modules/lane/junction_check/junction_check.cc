...
void JunctionCheck::Process(const std::vector<Group::Ptr>& groups) {
  if (groups.empty()) {
    junc_info_.road_scene_type = RoadSceneType::IN_JUNCTION;
      ...
      if (ego_group_index != -1) {
        ...
        if (end_po.x() > near_junction_dis_thresh_) {
          ...
          if (ego_group_index == static_cast<int>(groups.size()) - 1) {
            junc_info_.road_scene_type = RoadSceneType::NEAR_JUNCTION;
            junc_info_.along_path_vec = ego_pos - end_po;
          } else {
