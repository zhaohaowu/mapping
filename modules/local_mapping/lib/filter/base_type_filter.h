...
template<typename LineType, typename MeasureType, typename TargetType>
bool BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::Init(const FilterInitOption& init_options) {
  // todo 配置参数
...
// 统计连续多帧的检测类型count数
template<typename LineType, typename MeasureType, typename TargetType>
int BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::countContinueTypeNum(
    const MeasurementPtr& measurement) {
  int count = 1;
...
template<typename LineType, typename MeasureType, typename TargetType>
LineType BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::getTrackType(
    const MeasurementPtr& measurement) {
  LineType track_type =
      target_ref_->GetConstTrackedObject()->type;
  const LineType& detect_type = measurement->type;
...
template<typename LineType, typename MeasureType, typename TargetType>
void BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::UpdateWithMeasurement(
...
template<typename LineType, typename MeasureType, typename TargetType>
void BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::UpdateWithoutMeasurement(
    const FilterOption& filter_options) {
  // 没有检测时， 保持上一帧结果进行输出。
...
template<typename LineType, typename MeasureType, typename TargetType>
void BaseLaneLineTypeFilter<LineType, MeasureType, TargetType>::Reset() { return; }
