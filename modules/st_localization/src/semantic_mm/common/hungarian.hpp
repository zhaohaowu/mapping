...
class HungarianOptimizer {
 ...
 public:
  HungarianOptimizer() = default;
  ~HungarianOptimizer() = default;

  bool SetCosts(const std::vector<std::vector<T>>& cost_table);

  // @brief: find assignments to minimize costs
  bool Solve(std::vector<std::pair<size_t, size_t>>* assignments);

 private:
  enum class Mark { NONE = 0, PRIME, STAR };

  bool OptimizationInit();

  /* Convert the final cost matrix into a set of assignments of agents to
   * tasks.
  ...
  /* the number of stars in each column - used to speed up
   * coverStarredZeroes.*/
...
template <typename T>
bool HungarianOptimizer<T>::Solve(
...
template <typename T>
bool HungarianOptimizer<T>::OptimizationInit() {
  if (matrix_size_ == 0) return false;
  ...
  stars_in_col_.reserve(matrix_size_);
  rows_covered_.reserve(matrix_size_);
  cols_covered_.reserve(matrix_size_);
  uncov_row_.reserve(matrix_size_);
  uncov_col_.reserve(matrix_size_);
  assignments_.reserve(matrix_size_ * 2);
...
template <typename T>
bool HungarianOptimizer<T>::SetCosts(
    const std::vector<std::vector<T>>& cost_table) {
  size_t rows_size = cost_table.size();
  size_t cols_size = rows_size > 0 ? cost_table.at(0).size() : 0;
  width_ = cols_size;
  height_ = rows_size;
  matrix_size_ = std::max(rows_size, cols_size);

  costs_.resize(matrix_size_, matrix_size_);

  for (size_t row_idx = 0; row_idx < rows_size; ++row_idx) {
    if (cost_table.at(row_idx).size() != cols_size) {
      LC_LERROR(COMMON) << "hungarian gets non-rectangle input data";
      costs_.resize(0, 0);
      return false;
    }
    for (size_t col_idx = 0; col_idx < cols_size; ++col_idx) {
      costs_(row_idx, col_idx) = cost_table.at(row_idx).at(col_idx);
    }
  }
  return true;
}
...
void HungarianOptimizer<T>::MakeAugmentingPath() {
   ...
   * the first such star that we visit more than once; it must have been
   * reached
