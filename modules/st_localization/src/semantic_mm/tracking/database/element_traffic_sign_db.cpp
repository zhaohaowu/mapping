...
ElementTrafficSignDB::ElementTrafficSignDB(const CameraModel::Ptr& camera_model,
                                           const SE3d& T_veh_cam)
    : camera_model_(camera_model), T_veh_cam_(T_veh_cam) {}

void ElementTrafficSignDB::Reset() {
  ...
  // reset trackers
  sign_trackers_.clear();

  // reset tracking related data
  id_mapping_.clear();
  id_mapping_inverse_.clear();
  tracked_sign_data_.clear();
}

adLocStatus_t ElementTrafficSignDB::TrackingAndFusion(bool map_aided) {
  ...
  if (smm_param.track_by_percept_tracker_id) {
    ...
    MatchingManager::MatchingTrafficSignBbox(timestamp_, T_w_cam, percept_data_,
                                             tracked_sign_data_, sign_trackers_,
  ...
  for (const auto& item : percept_data_) {
    ...
    if (iter != matched_ids.end()) {
      id_t tracker_id = iter->second;
      auto tracker_iter = sign_trackers_.find(tracker_id);
      if (tracker_iter == sign_trackers_.end()) {
      ...
      LC_LDEBUG(TRACKING) << "new tracker sign " << new_tracker_id;
      sign_trackers_.insert(std::make_pair(new_tracker_id, new_tracker));
      tracked_sign_data_.insert(
          {new_tracker_id, std::make_shared<PerceptTrafficSign>(item.second)});
      if (smm_param.track_by_percept_tracker_id) {
  ...
  for (auto iter = sign_trackers_.begin(); iter != sign_trackers_.end();) {
    if (!iter->second->IsTrackerInBEV(T_w_cam)) {
      tracked_sign_data_.erase(iter->first);
      if (smm_param.track_by_percept_tracker_id) {
        id_t pid = id_mapping_inverse_[iter->first];
        id_mapping_.erase(pid);
        id_mapping_inverse_.erase(iter->first);
      }
      sign_trackers_.erase(iter++);
    } else {
      iter++;
    }
  }

  auto status = GetStableSigns();
  if (status != LOC_SUCCESS) {
    LC_LERROR(SMM) << "get stable signs failed!";
    return status;
  }
  return LOC_SUCCESS;
}

adLocStatus_t ElementTrafficSignDB::GetStableSigns() {
  fused_local_percept_data_.clear();
  for (auto iter = sign_trackers_.begin(); iter != sign_trackers_.end();
       iter++) {
    if (!iter->second->GetCenterValid()) continue;
    // TODO(xxx): fuse other attribute
    PerceptTrafficSign::Ptr fused_sign = std::make_shared<PerceptTrafficSign>();
    fused_sign->processed_center = iter->second->GetFusedCenter(frame_pose_);
    // TODO(xx): estimate the covariance
    fused_sign->processed_center_cov = Eigen::Matrix3d::Identity();
    fused_sign->processed_center_cov(1, 1) = 0.25;
    fused_sign->observ_cnt = iter->second->GetObservationCount();
    LC_LDEBUG(TRACKING) << "sign " << iter->first << " observ_cnt "
                        << fused_sign->observ_cnt;
    fused_local_percept_data_.insert({iter->first, fused_sign});
  }
  if (fused_local_percept_data_.empty()) return LOC_LOCALIZATION_ERROR;
  return LOC_SUCCESS;
}

adLocStatus_t ElementTrafficSignDB::UpdateTrackedPerceptData(
    const id_t tracker_id, const PerceptTrafficSign& new_percept) {
  auto tracker_iter = sign_trackers_.find(tracker_id);
  if (tracker_iter == sign_trackers_.end()) {
    LC_LERROR(SMM) << "no sign tracker " << tracker_id;
    return LOC_INVALID;
  }
  auto tracked_sign_iter = tracked_sign_data_.find(tracker_id);
  if (tracked_sign_iter == tracked_sign_data_.end()) {
    LC_LERROR(SMM) << "no tracked sign " << tracker_id;
    return LOC_INVALID;
  }
  ...
  if (!smm_param.track_by_percept_tracker_id) {
    ...
    if (delta_time_ms > 0.0 && delta_time_ms < 500.0) {
      Point2D_t curr_move_v =
          (new_percept.rect.center - tracked_sign_iter->second->rect.center) /
          (delta_time_ms / 100.0);
      Point2D_t last_move_v = tracked_sign_iter->second->move_velocity;
      tracked_sign_iter->second->move_velocity =
          curr_move_v * 0.7 + last_move_v * 0.3;
    } else {
      // get a large time gap, need recalculate the velocity
      tracked_sign_iter->second->move_velocity = Point2D_t(0.0, 0.0);
    }
  }

  // update basic info
  tracked_sign_iter->second->id = new_percept.id;
  tracked_sign_iter->second->confidence = new_percept.confidence;
  tracked_sign_iter->second->rect = new_percept.rect;
  tracked_sign_iter->second->type = new_percept.type;

  return LOC_SUCCESS;
}

adLocStatus_t ElementTrafficSignDB::BetweenTrackerFusion() {
  if (sign_trackers_.size() < 2) return LOC_SUCCESS;

  std::vector<id_t> all_tracker_ids;
  all_tracker_ids.reserve(sign_trackers_.size());
  for (const auto& tracker : sign_trackers_) {
  ...
  for (int i = 0; i < all_tracker_ids.size(); ++i) {
    ...
    const auto& curr_tracker = sign_trackers_[curr_id];
    ...
    for (int j = i + 1; j < all_tracker_ids.size(); ++j) {
      ...
      const auto& target_tracker = sign_trackers_[target_id];
      ...
      if ((target_center - curr_center).Norm2D() < 5.0) {
        ...
        const auto& bbox_curr = tracked_sign_data_[curr_id]->rect;
        const auto& bbox_tar = tracked_sign_data_[target_id]->rect;
  ...
  for (const auto& item : to_fuse_pairs) {
    ...
    auto& main_data = tracked_sign_data_[main_tracker_id];
    main_data = tracked_sign_data_[item.second.back()];

    auto& main_tracker = sign_trackers_[main_tracker_id];
    for (const auto& to_fuse_id : item.second) {
      ...
      main_tracker->Update(sign_trackers_[to_fuse_id]);
      sign_trackers_.erase(to_fuse_id);
      tracked_sign_data_.erase(to_fuse_id);
    }
