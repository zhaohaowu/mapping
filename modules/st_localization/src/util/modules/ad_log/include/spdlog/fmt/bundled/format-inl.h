...
FMT_FUNC int safe_strerror(int error_code,
                           char*& buffer,
...
FMT_FUNC void format_error_code(internal::buffer<char>& out,
                                int error_code,
...
FMT_FUNC void fwrite_fully(const void* ptr,
                           size_t size,
                           size_t count,
                           FILE* stream) {
...
digits::result grisu_gen_digits(fp value,
                                uint64_t error,
                                int& exp,
                                Handler& handler) {
...
FMT_API bool grisu_format(Double value,
                          buffer<char>& buf,
                          int precision,
                          unsigned options,
                          int& exp) {
    ...
    if (precision != -1) {
        ...
        if ((options & grisu_options::grisu3) != 0) {
            ...
            // value.
            grisu_shortest_handler<3> handler{buf.data(), 0,
                                              (upper - fp_value).f};
            ...
            grisu_shortest_handler<2> handler{buf.data(), 0,
                                              (upper - fp_value).f};
            result = grisu_gen_digits(upper, upper.f - lower.f, exp, handler);
            size = handler.size;
        }
...
template <typename Double>
char* sprintf_format(Double value,
...
FMT_FUNC void windows_error::init(int err_code,
                                  string_view format_str,
                                  format_args args) {
    error_code_ = err_code;
    memory_buffer buffer;
