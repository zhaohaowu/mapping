...
internal::truncating_iterator<OutputIt> printf(
    internal::truncating_iterator<OutputIt> it,
    basic_string_view<Char> format,
    basic_format_args<Context> args) {
...
class basic_printf_context {
 ...
 private:
    ...
    static void parse_flags(format_specs& specs,
                            const Char*& it,
                            const Char* end);
    ...
    unsigned parse_header(const Char*& it,
                          const Char* end,
                          format_specs& specs);
    ...
    while (it != end) {
        ...
        if (it != end && *it == '.') {
            ...
            if ('0' <= c && c <= '9') {
                internal::error_handler eh;
                specs.precision =
                    static_cast<int>(parse_nonnegative_int(it, end, eh));
            } else if (c == '*') {
                ++it;
                specs.precision = visit_format_arg(
                    internal::printf_precision_handler(), get_arg());
            } else {
                specs.precision = 0;
            }
        }
...
inline int fprintf(std::basic_ostream<Char>& os,
                   const S& format_str,
                   const Args&... args) {
    using context = basic_printf_context_t<Char>;
