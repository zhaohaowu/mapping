...
#include "modules/local_mapping/lib/laneline/tracker/bev_lane_tracker/data_fusion/laneline_point_filter.h"
...
void LanePointFilter::CreateAMatrix() {
...
void LanePointFilter::RevisePredictFit() {
...
void LanePointFilter::CalculateNormalV2() {
...
void LanePointFilter::UpdateWithoutMeasurement(
    const LaneFilterOptions& filter_options) {
  // 没有检测时， 保持上一帧结果进行输出。
  // PredictStage();
...
void LanePointFilter::CalculateNormal() {
  X_NORMAL_.setZero();
...
void LanePointFilter::PredictStage() {
  RevisePredictFit();
  A_update_ = B_ * A_;
  X_ = A_update_ * X_;
...
void LanePointFilter::Reset() { return; }
