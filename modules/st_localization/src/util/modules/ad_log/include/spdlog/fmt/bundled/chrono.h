...
namespace internal {
inline std::size_t strftime(char* str,
                            std::size_t count,
                            const char* format,
                            const std::tm* time) {
...
inline std::size_t strftime(wchar_t* str,
                            std::size_t count,
...
struct formatter<std::chrono::duration<Rep, Period>, Char> {
 private:
    ...
    struct spec_handler {
        ...
        make_arg_ref(basic_string_view<Char> arg_id) {
            context.check_arg_id(arg_id);
            const auto str_val =
                internal::string_view_metadata(format_str, arg_id);
 ...
 public:
    ...
    auto format(const duration& d, FormatContext& ctx) -> decltype(ctx.out()) {
        ...
        if (begin == end || *begin == '}') {
            out = internal::format_chrono_duration_value(out, d.count(),
                                                         precision);
            internal::format_chrono_duration_unit<Period>(out);
        } else {
            internal::chrono_formatter<FormatContext, decltype(out), Rep,
                                       Period>
                f(ctx, out, d);
