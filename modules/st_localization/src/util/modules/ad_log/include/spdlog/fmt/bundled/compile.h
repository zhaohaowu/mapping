...
class format_preparation_handler : public internal::error_handler {
 ...
 public:
    ...
    FMT_CONSTEXPR const Char* on_format_specs(const Char* begin,
        ...
        internal::dynamic_format_specs<Char> parsed_specs;
        dynamic_specs_handler<parse_context> handler(parsed_specs,
                                                     parse_context_);
        ...
        auto specs =
            last_part.which == part::kind::argument_id
...
class prepared_format {
            ...
            switch (part.which) {
                ...
                case format_part_t::kind::specification: {
                    ...
                    ctx.advance_to(visit_format_arg(
                        arg_formatter<Range>(ctx, nullptr, &specs), arg));
                } break;
            }
        }
 ...
 private:
    ...
    void format_arg(basic_parse_context<char_type>& parse_ctx,
                    Context& ctx,
                    Id arg_id) const {
...
FMT_CONSTEXPR PartsContainer prepare_parts(basic_string_view<Char> format) {
    ...
    internal::parse_format_string</*IS_CONSTEXPR=*/false>(
        format,
        format_preparation_handler<Char, PartsContainer>(format, parts));
    return parts;
}
...
std::basic_string<Char> format(const CompiledFormat& cf, const Args&... args) {
    ...
    cf.template vformat_to<range, context>(
        range(buffer), {make_format_args<context>(args...)});
...
std::size_t formatted_size(const CompiledFormat& cf, const Args&... args) {
    return fmt::format_to(internal::counting_iterator<
