...
class MessageBuffer {
 ...
 public:
  ...
  bool get_message_before_or_cur(const double& timestamp, MessageType* msg);
  bool get_messages_after(const double& timestamp, MessageType* msg);
  bool get_all_messages_after(const double& timestamp,
                              std::map<double, MessageType>* msg);

  bool get_all_messages_after_end(const double& after, const double& end,
                                  std::vector<MessageType>* msg);
...
bool MessageBuffer<MessageType>::get_message_before(const double& timestamp,
  ...
  for (auto iter = msg_list.rbegin(); iter != msg_list.rend(); ++iter) {
    // HLOG_FATAL << "iter->first " << iter->first;
    // HLOG_FATAL << "iter->second->timestamp " << iter->second->timestamp;
...
template <class MessageType>
bool MessageBuffer<MessageType>::get_message_before_or_cur(
...
template <class MessageType>
bool MessageBuffer<MessageType>::get_messages_after(const double& timestamp,
                                                    MessageType* msg) {
  _buffer_mutex.lock();
  if (_msg_list.empty()) {
    _buffer_mutex.unlock();
...
template <class MessageType>
bool MessageBuffer<MessageType>::get_all_messages_after(
    const double& timestamp, std::map<double, MessageType>* msg) {
  _buffer_mutex.lock();
  if (_msg_list.empty()) {
    _buffer_mutex.unlock();
    // ::std::cerr << "The buffer is empty." << ::std::endl;
  ...
  ::std::copy(this->_msg_list.begin(), this->_msg_list.end(),
              ::std::back_inserter(msg_list));
  _buffer_mutex.unlock();
  if (msg_list.rbegin()->first < timestamp) {
    HLOG_WARN << "get_all_messages_after faild";
    return false;
  }
  for (auto iter = msg_list.begin(); iter != msg_list.end(); ++iter) {
    if (iter->first >= timestamp) {
      msg->insert({iter->first, iter->second});
    }
  }
  return true;
}
...
template <class MessageType>
bool MessageBuffer<MessageType>::get_all_messages_after_end(
    const double& after, const double& end, std::vector<MessageType>* msg) {
  _buffer_mutex.lock();
  if (_msg_list.empty()) {
    _buffer_mutex.unlock();
    // ::std::cerr << "The buffer is empty." << ::std::endl;
    // LOCAL_LOG_ERROR_FORMAT("The buffer is empty.");
    HLOG_ERROR << "buffer is null";
    return false;
  }
  ...
  ::std::copy(this->_msg_list.begin(), this->_msg_list.end(),
              ::std::back_inserter(msg_list));
  _buffer_mutex.unlock();
  if (msg_list.rbegin()->first < after) {
    HLOG_WARN << "get_all_messages_after_end faild";
    return false;
  }
  for (auto iter = msg_list.begin(); iter != msg_list.end(); ++iter) {
    if (iter->first >= after && iter->first <= end) {
      msg->push_back(iter->second);
    }
  }
  return true;
}
