...
#include "modules/map_fusion/modules/lane/path_manager.h"

namespace hozon {
namespace mp {
namespace mf {

void PathManager::Init(const LaneFusionProcessOption& options) {
  conf_ = options;
  HLOG_INFO << "Path manager init";
}
...
void PathManager::AddPose(const KinePose& pose) {
  ...
  while (back_length_ > conf_.path_back_range && poses_.size() >= 2) {
...
// void PathManager::CheckPoseState(const KinePosePtr& cur_p) {
...
void PathManager::GetPath(std::vector<KinePosePtr>* path) {
  if (path == nullptr) {
    return;
  }

  KinePosePtr pose_back;

  {
    std::unique_lock<std::mutex> lock(path_mutex_);
    pose_back = std::make_shared<KinePose>(*(poses_.back()));
  ...
  if (path->empty() || conf_.path_predict_range <= 0) {
    return;
  }
  ...
  std::vector<KinePosePtr> last_n_poses(path->end() - avg_n, path->end());
  float length = 0;
  ...
  int predict_counts =
      static_cast<int>(conf_.path_predict_range / avg_interval);
  auto last_quat = pose_back->quat;
  auto last_stamp = pose_back->stamp;
  // 使用匀速匀旋转来递推位姿
  //! TBD:
  ...
  for (int i = 0; i < predict_counts; ++i) {
    ...
    Eigen::Vector3f p_ndt = pose_back->TransformPoint(dp_ndt);
...
KinePosePtr PathManager::LatestPose() {
