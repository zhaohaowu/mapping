...
#undef Z16
        ...
        while (ScanWriteUnescapedString(is, length)) {
            ...
            if (!TargetEncoding::supportUnicode &&
                ...
                if (codepoint <= 0xD7FF ||
                    ...
                    CEREAL_RAPIDJSON_ASSERT(codepoint >= 0x010000 &&
                                            codepoint <= 0x10FFFF);
                    // Surrogate pair
    ...
    void Prefix(Type type) {
        (void)type;
        if (CEREAL_RAPIDJSON_LIKELY(level_stack_.GetSize() !=
            ...
            if (!level->inArray && level->valueCount % 2 == 0)
                CEREAL_RAPIDJSON_ASSERT(
                    type == kStringType);  // if it's in object, then even
    ...
    bool EndValue(bool ret) {
        if (CEREAL_RAPIDJSON_UNLIKELY(
                level_stack_.Empty()))  // end of json text
            os_->Flush();
        return ret;
    }
...
inline bool Writer<StringBuffer>::ScanWriteUnescapedString(StringStream& is,
                                                           size_t length) {
    if (length < 16) return CEREAL_RAPIDJSON_LIKELY(is.Tell() < length);

    if (!CEREAL_RAPIDJSON_LIKELY(is.Tell() < length)) return false;
    ...
    for (; p != endAligned; p += 16) {
        ...
        if (CEREAL_RAPIDJSON_UNLIKELY(r !=
                                      0)) {  // some of characters is escaped
            SizeType len;
