...
#include <utility>
// #include
// "modules/local_mapping/lib/laneline/tracker/bev_lane_tracker/data_fusion/laneline_quality_filter.h"
...
void SetLanePosition(
  ...
  for (int i = 0; i < left_lane_index.size(); ++i) {
    ...
    lane_result_pos_map->emplace(std::make_pair(
        lane_lines[lane_index]->id, lane_lines[lane_index]->position));
  }
