...
namespace mp {
namespace lm {
bool RoadEdgePointFilter::Init(const FilterInitOption& init_options) {
  // 点更新管理器初始化
...
bool RoadEdgePointFilter::ConvertEigen2PointSet(
  ...
  track_roadedge->vehicle_points.resize(20);
  track_roadedge->world_points.resize(20);
  assert(eigen_vector.rows() / 2 == track_roadedge->vehicle_points.size());

  const auto& current_pose = PoseManager::Instance()->GetCurrentPose();

  Eigen::Vector3d local_pt(0.0, 0.0, 0.0);
  Eigen::Vector3d vehicle_pt(0.0, 0.0, 0.0);
  for (int64_t idx = 0; idx < track_roadedge->world_points.size(); ++idx) {
    auto& local_point = track_roadedge->world_points[idx];
    auto& vehicle_point = track_roadedge->vehicle_points[idx];
    local_point[0] = eigen_vector[2 * idx];
    local_point[1] = eigen_vector[2 * idx + 1];
    local_point[2] = 0.0;
    vehicle_point = current_pose.inverse() * local_point;
  }
  return true;
}

void RoadEdgePointFilter::MergeMapTrackLanePoints() {
