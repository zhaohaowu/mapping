...
void FusionCenter::UpdateLatestFc() {
  ...
  if (fc_map_[latest_imu_.timestamp].timestamp - latest_mm_.timestamp < 3) {
    fc_state.state = 2;
  } else {
    fc_state.state = 5;
  }
...
Measure::ConstPtr FusionCenter::GetNewIns(
    const Measure::ConstPtr& cur_ins, const Measure& latest_mm,
    const Eigen::Vector3d& fc_ref_point_) {
  Measure::Ptr new_ins = std::make_shared<Measure>(*cur_ins);
  // 将mm转到enu系下
  Eigen::Vector3d mm_position =
      util::Geo::Gcj02ToEnu(latest_mm.gcj_position, fc_ref_point_);
  Eigen::Affine3d T_w_mm =
      Eigen::Translation3d(mm_position) * Eigen::Affine3d(latest_mm.enu_q);
  Eigen::Affine3d T_w_ins = Eigen::Translation3d(cur_ins->enu_position) *
                            Eigen::Affine3d(cur_ins->enu_q);
...
void FusionCenter::StateMeasure(const Measure::ConstPtr& measure,
  ...
  if (measure->measure_type == MM || measure->measure_type == INS) {
    ...
    // 误差状态更新
    Eigen::Matrix<double, 9, 3> K = fc_state->P * H.transpose() *
                                    (H * fc_state->P.eval() * H.transpose() +
                                     fc_state->q * fc_state->R_chassis)
                                        .inverse();
